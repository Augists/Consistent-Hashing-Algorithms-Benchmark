...
func (mpch *MPCH) GetNode(key string) string {
	...
	// 预先计算所有节点的哈希值
	nodeHashes := make([]uint64, len(mpch.nodes))
	for i, node := range mpch.nodes {
		nodeHashes[i] = mpch.hash(node, 0)
	}
	...
	for i := 0; i < mpch.k; i++ {
		...
		// 对每个节点计算得分
		for j, nodeHash := range nodeHashes {
