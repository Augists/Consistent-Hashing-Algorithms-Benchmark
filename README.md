...
| 算法                  | 耗时     | 说明           |
|-----------------------|----------|----------------|
| 直接哈希取模          | 10.644ms | 最快的查询速度 |
| Maglev哈希(2039表长)  | 12.074ms | 快速查询       |
| Maglev哈希(65537表长) | 12.459ms | 快速查询       |
| 跳跃一致性哈希        | 17.629ms | 快速查询       |
| 哈希环(40个虚拟节点)  | 19.617ms | 中等查询速度   |
| 哈希环(160个虚拟节点) | 23.178ms | 中等查询速度   |
| AnchorHash(2000长度)  | 64.653ms | 中等查询速度   |
| DxHash                | 36.642ms | 中等查询速度   |
| Rendezvous哈希        | 12.450s  | 查询速度较慢   |
| Multi-Probe(k=5)      | 13.756s  | 查询速度较慢   |
| Multi-Probe(k=21)     | 16.820s  | 查询速度最慢   |

### 添加节点重映射测试 (1000初始节点+10新增节点)

| 算法                  | 重映射键数 | 重映射比例 | 添加节点耗时 | 说明               |
|-----------------------|------------|------------|--------------|--------------------|
| 直接哈希取模          | 98971      | 98.97%     | 18.167µs     | 几乎全部需要重映射 |
| AnchorHash(2000长度)  | 717        | 0.72%      | 14.291µs     | 最小化重映射       |
| 跳跃一致性哈希        | 969        | 0.97%      | 2.75µs       | 最小化重映射       |
| Rendezvous哈希        | 977        | 0.98%      | 1.610ms      | 最小化重映射       |
| Multi-Probe(k=21)     | 989        | 0.99%      | 8.875µs      | 最小化重映射       |
| Multi-Probe(k=5)      | 1003       | 1.00%      | 8.666µs      | 最小化重映射       |
| DxHash                | 1297       | 1.30%      | 11.541µs     | 较少重映射         |
| 哈希环(40个虚拟节点)  | 953        | 0.95%      | 45.486ms     | 最小化重映射       |
| 哈希环(160个虚拟节点) | 1078       | 1.08%      | 193.207ms    | 较少重映射         |
| Maglev哈希(2039表长)  | 3504       | 3.50%      | 3.261ms      | 中等重映射         |
| Maglev哈希(65537表长) | 3418       | 3.42%      | 30.797ms     | 中等重映射         |
