...
<!--
首先，很不好意思耽误大家休息时间来听我分享这个。因为内容比较多，有些地方理解起来也比较费劲，一些算法我自己理解也废了很大劲。我尽量把这些算法的核心思想抽离出来让它更容易听懂一点。在过程中大家有遇到想问的问题可以直接打断我。希望我讲的这个能帮助大家拓展一些思路

我觉得，LB 算法就是一个一直在讨论公平性的问题。我们会需要从不同的角度看待问题：client 要看自己能不能及时访问到服务、LB 要看自己怎么分配、RS 要一直有能力能及时处理
...
相比于收集 RS 还有多少任务要处理，或是持续统计所有 RS 的 CPU 占用率这样的方式

肯定是直接由 RS 性能来定义权重最为简单高效
...
<!--
这里我按照LVS的WRR实现列出了它实际的调度顺序的结果

以两个服务器权重分别为1和5为例，这样GCD是1
...
<!--
然后更新所有服务器的当前权重，在之前的基础上增加定义的权重

继续下一次选择，这里A和B都是最大权重，我们选择了更靠前的A
-->
...
我在这里放了它的计算和调度两部分的伪代码

上面是调度顺序表的计算，它在拿到下一个SWRR的结果之后就会把它放进vrs table里

下面实际调度的时候，只需要顺序的遍历vrs table表就可以得到调度顺序
-->
...
<!--
可以看到在计算的循环里，idx只是用来表示在表中下一个记录的位置，和实际的调度计算无关。这也就意味着它是可以随时暂停的
-->
...
首先，在我们当前的实现里，会在前几次调度时查看并计算还未计算的Step。这其实会影响前几次调度的延迟，等于前几次调度的时候把整个调度顺序表计算出来，相比不划分Step的完整计算区别不大，甚至还把原本CTRL的计算开销放到了WORKER里。假设放大到所有WORKER、整个LB集群，就不是很优雅的分步计算方案了
-->
...
<!--
我们在测试性能的时候，通常会关注三个指标：PPS（包）、CPS（连接数）、BPS（带宽）。前面的都是从数据包（连接）层面上看调度，现在我们从服务器连接状态的角度看LB
...
<!--
想要再把权重纳入考虑，不同的实现里采用了不同方法

DPVS使用了第二种方案，除权重

Nginx直接采用第三种：两两对比乘对方的权重，O(N)的比较出选择结果
-->
...
<!--
这里按照时间顺序整理了一些较为通用或较新的一致性哈希算法。最后一个Rendezvous哈希比较特殊，我们先从它开始
-->
...
另一点是扩容时会影响一些key的第一选择，但是如果我们不在乎保持的话也是可以接受的。但是如果想要维护映射关系，就需要把所有key都拿出来重新算一遍，非常麻烦
-->
...
<!--
考虑哈希环算法的实现。针对查找开销的问题，预先填充是不可行的
...
<!--
哈希环和MPCH总会给人一种很笨重的感觉。而Jump Hash算法实现了一种非常轻量的负载均衡。 Google在2014年可能是最早的将伪随机的思想应用到这里，提出了跳跃一致性哈希算法
...
伪随机的作用是，同一个key不管是什么时候，生成的随机数序列是固定的，那么它们发生重映射的时间（即在添加第几个槽位的时候发生了重映射）就是确定的
-->
...
在这种实现里，它需要O(N)即RS数量的时间确定它该不该跳到下一个RS的槽位里
-->
...
<!--
所以在优化的版本里，随机值的含义变了，j从原本的循环N次变成了跳跃式的遍历，这里我们随机出来的结果直接用来计算下一次会发生跳跃的位置。最终实现了O(logN)时间的查询
-->
...
<!--
Jump Hash是一种看懂之后会赞叹很妙的算法，它在。。。要好。但是它对RS的定位需要依靠idx标号，需要把RS以数组形式存放

在负载均衡调度的场景下，如果一个连接建立后断开，下一次又来建立连接，就要看是否需要重回到之前的调度，这也是一致性哈希想要控制的问题。在例如分布式数据库的场景下，就必须要对之前已有的key映射做维护。但是负载均衡场景并不需要对持久化做 100% 保证，那么跳跃一致性哈希就不需要维护 key 的移动，仅仅需要使用  JumpConsistentHash 选择出调度的 RS 即可
-->
...
并且我们尝试维护总RS数量和当前存活的RS数量，当Jump Hash映射到了一个not alived状态的RS时，就根据当前存活的数量来保证一定可以找到一个可行的调度结果。这有点类似后面要介绍的AnchorHash的思想

最后，我们也不能放任删除的RS不管，这样如果变动较多，RS数组就会无限增长了。所以我们在。。。
-->
...
可以看右边的查找表建立过程。我们按照B0 B1 B2的顺序开始填充，B0 0位置的偏好是3，查找表3位置是空的，所以就可以填充B0。然后B1填充到0位置，到B2的时候，也是位置3，但是3位置已经被B0占了，所以就会向下顺眼找到它下一个偏好4，在4位置填充。然后回到B0继续查看偏好序列，发现0和4位置都被占了，就继续向后查找偏好序列。这样填充直到查找表被填满为止
-->
...
需要注意的是，Maglev虽然避免了hash取模的全局重映射，但是它并不是最小化的重映射。这个我们下一页ppt再说

先说查找表的生成时间测试。Google尝试了两个不同大小的质数作为查找表的长度，通过增大查找表长度来保证均衡性更高。。。
-->
...
5. 负载的分配较均匀，方差较低。但实际上在后面的实验里我们会发现它的负载平衡性非常差。这个的原因是，在官方提供的源码实现里，A\W部分的回填都会指向最后一个工作节点。也就是说在不发生RS增减时，会有A-W/W比例的key映射到最后一个节点，而非它算法所说的根据W节点数量重新随机。所以从它源码实现的角度来理解，AnchorHash更适合槽位只在小范围变动且W数量接近A的场景
...
<center>
<img src="./media/分布均匀性测试.png" width="40%" />
</center>
...
<!--
然后我们测试了在添加了1k个节点以后，调度查询10w个key所花费的时间。前三行是O1的查找，时间区别主要在查找表长度
...
<!--
作为对比，我就尝试统计了目前DGW进行调度的时间。在21服务器上启动DGW，然后由10000个clients向100个servers发送简单的tcp包

最终几种调度方式计算平均时间后扩大到10w个key，总花费在2ms多，minConn需要12ms多。结果不一致很大一部分原因是实验是用go实现的，且实现方式上会有一些区别
-->
...
<!--
然后是在添加了1000个初始节点之后，尝试再添加10个节点，看10w个key中有多少会发生重映射
...
<center>
<img src="./media/性能综合对比表.png" width="58%" />
</center>
...
阿里云只在 CHash 部分提到了会话保持，并说明是哈希特性自带，并没有说明有提供类似 sticky 的保持能力，所以我理解阿里云并不提供 sticky 能力，扩缩容时会导致一部分请求需要重新分配。华为云提到说所有算法都支持了会话保持。在DGW的实现里取消了除哈希以外的算法的sticky

美团公开的文档里核心关注的点不在负载均衡调度，但也详细的介绍了它们实现了哈希环算法，并提了一句有关注到Maglev

在剩下的这些里面，比较重要的是DPVS和Nginx，其他的实现仅作为思路拓展
-->
...
<!--
Nginx作为提出SWRR算法的，它也把这个作为了默认和所有其他算法的兜底，其他算法一旦调度失败都会回到SWRR调度。并且它所有的算法把权重纳入了调度考虑

在源码http upstream部分我找到了SWRR，加权最小连接，加权ip哈希，加权哈希环和完全随机的调度实现
-->
...
<!--
Nginx在加权哈希环中对每个节点创建了权重乘160数量的影子节点来保证负载的均衡性，这会开辟非常大片的内存资源

Nginx比较特殊的两点是：ip hash取模了总权重，按顺序减掉每一个RS的权重，来判断结果落在哪个RS。这相当于把所有RS根据他们的权重平铺在一个数组中
...
注意到高亮部分的处理比较特殊。一个是它会每一次大循环遍历都累加出总权重，另一个是它的定义权重是会变化的
-->
...
在系统初始时，通过将所有 effective_weight 初始化为 1 来解决 SWRR 的羊群效应并放大随机效果
...
<!--
这里peer weight是用户配置定义的权重，effective weight其实也是，但是是一个动态变化的有效权重值，初始化为weight。在Nginx的设计里，当发生通信错误时就会减小，来降低继续向这个节点转发的可能，然后逐步的恢复
...
```c [weight start from 1 & random choice] {all|1-2|3,14|all}
...
我们将effective weight初始化为1，weight初始化为定义值

然后引入随机数来判断，当比较的两个RS当前权重相等时，只有随机结果超过阈值才会更新best，从而打乱调度顺序

其他部分仿照Nginx，逐步更新effective weight和更新best的当前权重值
-->
...
这个SWRR方案的好处是实现逻辑简单、解决了集群内统一调度的问题，但是这没有实现VNSWRR的预计算的O(1)调度
-->
...
<!--
这段代码是对延迟的时间衰减，用时间间隔来计算衰减系数，从而减少之前的结果的影响
-->
...
<!--
best of two随机选择我没有去看它原本的论文，但是大意上就是从所有RS里开始随机选择出两个，然后根据他们的权重分数来找到两个中更好的那个做转发
-->
